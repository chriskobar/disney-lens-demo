...
// Uses import instead of fs.readFileSync so Vercel bundles the files.

import characterConfigData from '../character-config.json';

// Story config: use dynamic require in a try/catch so the app
// doesn't crash if the file is removed later.
let storyConfig = null;
try {
  storyConfig = require('../story-config.json');
} catch (err) {
  console.warn('story-config.json not found, story layer disabled');
}

const config = characterConfigData;
